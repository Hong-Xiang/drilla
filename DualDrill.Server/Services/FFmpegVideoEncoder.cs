...
{
    public readonly Memory<byte> Memory = Data.AsMemory()[..Length];
    public void Dispose()
    {
        Pool.Return(Data);
    }

    public static VideoFrameBuffer Create(ArrayPool<byte> pool, int length)
    {
        var data = pool.Rent(length);
        return new VideoFrameBuffer(data, length, pool);
    }
}

public sealed unsafe class DrillFFmpegVideoEncoder : IDisposable
{
    ...
    public DrillFFmpegVideoEncoder(Dictionary<string, string>? encoderOptions = null, AVHWDeviceType HWDeviceType = AVHWDeviceType.AV_HWDEVICE_TYPE_NONE)
    {
    ...
    public VideoFrameBuffer? EncodeVideo(int width, int height, ReadOnlySpan<byte> sample, VideoPixelFormatsEnum pixelFormat, VideoCodecsEnum codec)
    {
    ...
    public VideoFrameBuffer? EncodeVideoFaster(RawImage rawImage, VideoCodecsEnum codec)
    {
        byte* pSample = (byte*)rawImage.Sample;
        return EncodeVideo(rawImage.Width, rawImage.Height, pSample, rawImage.PixelFormat, codec);
    }

    private VideoFrameBuffer? EncodeVideo(int width, int height, byte* sample, VideoPixelFormatsEnum pixelFormat, VideoCodecsEnum codec)
    {
        ...
        return Encode(codecID.Value, sample, width, height, 60, true, avPixelFormat);
    }
    ...
    public VideoFrameBuffer? Encode(AVCodecID codecID, byte* sample, int width, int height, int fps, bool keyFrame = false, AVPixelFormat pixelFormat = AVPixelFormat.AV_PIX_FMT_YUV420P)
    {
        lock (_encoderLock)
        {
            if (!_isDisposed)
    ...
    public VideoFrameBuffer? Encode(AVCodecID codecID, AVFrame avFrame, int fps, bool keyFrame = false)
    {
        if (!_isDisposed)
        {
            lock (_encoderLock)
            {
                ...
                try
                {
                    ...
                    if (error == 0)
                    {
                        if (_codecID == AVCodecID.AV_CODEC_ID_H264)
                        {
                            var length = pPacket->size;
                            var buffer = VideoFrameBuffer.Create(BufferPool, length);
                            //byte[] arr = new byte[pPacket->size];
                            var source = new Span<byte>(pPacket->data, length);
                            var target = buffer.Memory.Span;
