...
sealed class SharedBuiltinSymbolTable2 : ISingleton<SharedBuiltinSymbolTable2>
    , ISymbolTableView2
{
    public FrozenDictionary<IShaderSymbol, IShaderType> RuntimeTypes { get; }
    public FrozenDictionary<IShaderSymbol, ShaderDecl> RuntimeMethods { get; }

    SharedBuiltinSymbolTable2()
    {
        RuntimeTypes = GetRuntimeTypes().ToFrozenDictionary();
        RuntimeMethods = GetRuntimeMethods(RuntimeTypes).ToFrozenDictionary();
    }

    private Dictionary<IShaderSymbol, IShaderType> GetRuntimeTypes()
    {
        ...
        return result.ToDictionary(kv => new ShaderTypeSymbol(kv.Key) as IShaderSymbol, kv => kv.Value);
    }
    ...
    private Dictionary<IShaderSymbol, ShaderDecl>
        GetRuntimeMethods(
        IReadOnlyDictionary<IShaderSymbol, IShaderType> runtimeTypes)
    {
        ...
        //        if (runtimeTypes.TryGetValue(new ShaderTypeSymbol(returnType), out var rt))
        //        {
        //            var parameters = m.GetParameters();
        //            var paramTypes = parameters.Select(p => new ShaderTypeSymbol(p.ParameterType)).ToArray();
    ...
    public IShaderType? GetShaderType(IShaderSymbol symbol)
    {
        throw new NotImplementedException();
    }

    public static SharedBuiltinSymbolTable2 Instance { get; } = new SharedBuiltinSymbolTable2();
}