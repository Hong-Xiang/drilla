...
public sealed record class RegionTree<TL, TB>(IRegionDefinition<TL, Seq<RegionTree<TL, TB>, TB>> Definition)
{
    ...
    public static RegionTree<TL, TB> Block(TL label, ReadOnlySpan<RegionTree<TL, TB>> regions, TB body, TL? next) =>
        new(new BlockRegionDefinition<TL, Seq<RegionTree<TL, TB>, TB>>(label,
            Seq.Create([.. regions], body),
            next));

    public static RegionTree<TL, TB> Loop(TL label, ReadOnlySpan<RegionTree<TL, TB>> regions, TB body, TL? next,
        TL? breakNext) =>
        new(new LoopRegionDefinition<TL, Seq<RegionTree<TL, TB>, TB>>(label,
            Seq.Create([.. regions], body),
            next, breakNext));

    public RegionTree<TL, TB> WithBindings(IEnumerable<RegionTree<TL, TB>> regions)
    {
        return new RegionTree<TL, TB>(Definition.Select(seq => Seq.Create([.. regions], seq.Last)));
    }
    ...
    private sealed class FoldLazyImplSemantic<TS, T>(IRegionTreeFoldLazySemantic<TL, TB, TS, T> semantic)
        ...
        , ISeqSemantic<RegionTree<TL, TB>, TB, Func<TS>, Func<TS>>
    {
        ...
        {
            return () => semantic.Block(label, body.Fold(this), next);
        }

        public Func<T> Loop(TL label, Seq<RegionTree<TL, TB>, TB> body, TL? next, TL? breakNext)
        {
            return () => semantic.Loop(label, body.Fold(this), next, breakNext);
        }

        public Func<TS> Nested(RegionTree<TL, TB> head, Func<TS> next)
        {
            return () => semantic.Nested(head.Definition.Evaluate(this), next);
        }

        public Func<TS> Single(TB value)
        {
            return () => semantic.Single(value);
        }
    }


    private sealed class FoldImplSemantic<TS, T>(IRegionTreeFoldSemantic<TL, TB, TS, T> semantic)
        ...
        , ISeqSemantic<RegionTree<TL, TB>, TB, Func<TS>, TS>
    {
        ...
        public TS Single(TB value) => semantic.Single(value);
    }


    private sealed class MapFoldSemantic<TLR, TBR>(Func<TL, TLR> l, Func<TB, TBR> b)
        : IRegionTreeFoldSemantic<TL, TB, Seq<RegionTree<TLR, TBR>, TBR>, RegionTree<TLR, TBR>>
    {
        public RegionTree<TLR, TBR> Block(TL label, Func<Seq<RegionTree<TLR, TBR>, TBR>> body, TL? next)
        {
            ...
            return RegionTree<TLR, TBR>.Block(l(label), [.. bd.Elements], bd.Last,
                next is not null ? l(next) : default);
        }

        public RegionTree<TLR, TBR> Loop(TL label, Func<Seq<RegionTree<TLR, TBR>, TBR>> body, TL? next, TL? breakNext)
        {
            var bd = body();
            return RegionTree<TLR, TBR>.Loop(l(label), [.. bd.Elements], bd.Last, next is not null ? l(next) : default,
                breakNext is not null ? l(breakNext) : default);
        }

        public Seq<RegionTree<TLR, TBR>, TBR> Nested(RegionTree<TLR, TBR> head,
            Func<Seq<RegionTree<TLR, TBR>, TBR>> next) =>
            Seq<RegionTree<TLR, TBR>, TBR>.Nested(head, next());

        public Seq<RegionTree<TLR, TBR>, TBR> Single(TB value) => Seq<RegionTree<TLR, TBR>, TBR>.Single(b(value));
    }
}
...
public static class RegionTree
{
    ...
    public static RegionTree<TL, TB> Loop<TL, TB>(TL label, ReadOnlySpan<RegionTree<TL, TB>> regions, TB body, TL? next,
        TL? breakNext) =>
        RegionTree<TL, TB>.Loop(label, regions, body, next, breakNext);

    public static void Traverse(this RegionTree<Label, ShaderRegionBody> region,
        Action<RegionTree<Label, ShaderRegionBody>> f)
    {
        f(region);
    ...
    : IRegionTreeFoldSemantic<string, IEnumerable<string>, IEnumerable<string>, IEnumerable<string>>
{
    ...
    public IEnumerable<string> Nested(IEnumerable<string> head, Func<IEnumerable<string>> next) => [.. head, .. next()];

    public IEnumerable<string> Single(IEnumerable<string> value)
    {
        return value.Select(v => IndentStr + v);
    }
}

internal sealed class RegionDefinitionDefinedLabelsSemantic<TB>
    : IRegionTreeFoldSemantic<Label, TB, IEnumerable<Label>, IEnumerable<Label>>
{
    public IEnumerable<Label> Block(Label label, Func<IEnumerable<Label>> body, Label? next) => [label, .. body()];

    public IEnumerable<Label> Loop(Label label, Func<IEnumerable<Label>> body, Label? next, Label? breakNext) =>
        [label, .. body()];

    public IEnumerable<Label> Nested(IEnumerable<Label> head, Func<IEnumerable<Label>> next) => [.. head, .. next()];
...

{
    ...
    public static string Show(this RegionTree<Label, ISuccessor> region)
    {
        ...
        var sir = region.Select<string, IEnumerable<string>>(stepFormatter.LabelName,
            s => [s.Evaluate(stepFormatter, default)]);
        var lines = sir.Fold(new RegionDefinitionFormatter(0));
        return string.Join(Environment.NewLine, lines);
    }

    private sealed class SuccessorFormatSemantic(IReadOnlyDictionary<Label, int> labels)
