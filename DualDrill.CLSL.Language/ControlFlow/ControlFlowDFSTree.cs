...
public sealed class ControlFlowDFSTree
{
    ...
    public ControlFlowDFSTree(IControlFlowGraph controlFlowGraph)
    {
        ...
        void Visit(Label l)
        {
            ...
            {
                throw new InvalidOperationException($"Multiple visiting {l}");
            }
            visited.Add(l);
            HashSet<Label> c = [];
        ...
        {
            labels[LabelCount - kv.Value - 1] = kv.Key;
        }
        Labels = [.. labels];
        ...
        LabelLoopFlags = Labels.ToFrozenDictionary(l => l, l => controlFlowGraph.GetPred(l).Any(p => GetIndex(p) >= GetIndex(l)));
    }
    ...
    public bool IsLoop(Label label)
        => LabelLoopFlags[label];

    public T Fold<T>(IFoldSemantic<T, T> semantic)
        => FoldLabel(semantic, ControlFlowGraph.EntryLabel);
...
public static partial class ControlFlowExtension
{
    public static ControlFlowDFSTree GetDFSTree(this IControlFlowGraph controlFlowGraph)
