    ...
    , ITerminatorSemantic<RegionJump, IShaderValue, IEnumerable<IShaderValue>>
{
    ...
    public IEnumerable<IShaderValue> Loop(Label label, Func<IEnumerable<IShaderValue>> body, Label? next,
        Label? breakNext) =>
        body();
    ...
    public IEnumerable<IShaderValue> Dup(IShaderValue result, IShaderValue source) => [source];

    public IEnumerable<IShaderValue> Get(IShaderValue result, IShaderValue source) => [source];


    public IEnumerable<IShaderValue> Literal<TLiteral>(TLiteral literal) where TLiteral : ILiteral => [];

    public IEnumerable<IShaderValue> Mov(IShaderValue target, IShaderValue source) => [source];

    public IEnumerable<IShaderValue> Nop() => [];

    public IEnumerable<IShaderValue> Operation1(IUnaryExpressionOperation operation, IEnumerable<IShaderValue> e) => e;

    public IEnumerable<IShaderValue> Operation2(IBinaryExpressionOperation operation, IEnumerable<IShaderValue> l,
        IEnumerable<IShaderValue> r) => [.. l, .. r];

    public IEnumerable<IShaderValue> Pop(IShaderValue target) => [target];

    public IEnumerable<IShaderValue> Set(IShaderValue target, IShaderValue source) => [target, source];

    public IEnumerable<IShaderValue> SetVecSwizzle(IVectorSwizzleSetOperation operation, IShaderValue target,
        IShaderValue value) => [target, value];

    public IEnumerable<IShaderValue> Value(IShaderValue value) => [value];

    public IEnumerable<IShaderValue> VectorCompositeConstruction(VectorCompositeConstructionOperation operation,
        IEnumerable<IEnumerable<IShaderValue>> arguments)
    {
        return arguments.SelectMany(a => a);
    }
}
